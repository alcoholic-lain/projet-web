document.getElementById("registerForm").addEventListener("submit", function(e) {
    ...
    if (psw === "") {
        ...
        createLabel(pswInput, "Le mot de passe doit contenir au moins 8 caractères", false);
        isFormValid = false;
    }
    else if (!/[A-Z]/.test(psw)) {
        createLabel(pswInput, "Le mot de passe doit contenir au moins une majuscule", false);
        isFormValid = false;
    }
    else if (!/[a-z]/.test(psw)) {
        createLabel(pswInput, "Le mot de passe doit contenir au moins une minuscule", false);
        isFormValid = false;
    }
    else if (!/[0-9]/.test(psw)) {
        createLabel(pswInput, "Le mot de passe doit contenir au moins un chiffre", false);
        isFormValid = false;
    }
    else if (!/[!@#$%^&*()_\-+=<>?]/.test(psw)) {
        createLabel(pswInput, "Le mot de passe doit contenir au moins un symbole spécial (!@#$%^&*)", false);
        isFormValid = false;
    }
    else {
        createLabel(pswInput, "Mot de passe sécurisé ✅", true);
    }
    ...
    // Envoi si valide
    if(isFormValid){
        fetch("http://localhost/projet-web/controller/components/login/RegisterController.php", {
            method: 'POST',
