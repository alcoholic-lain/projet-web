...
class Message
{
    ...
    public function setConversationId(int $cid): void { $this->conversation_id = $cid; }
    ...
    // For admin: all messages as objects
    ...
    // For chat: messages with username
    public static function findByConversation(int $conversationId, int $limit = 200): array
    {
        $pdo = Database::getConnection();
        $sql = "
            SELECT m.id, m.conversation_id, m.user_id, m.content, m.created_at, u.username
            FROM messages m
